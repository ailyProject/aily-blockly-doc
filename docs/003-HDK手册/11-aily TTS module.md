...
| 调节内容 | 指令 | 说明                                                         |
| -------- | ---- | ------------------------------------------------------------ |
| FD 00 01 |      | 支持10级音量调节，发送协议头+[v0]-[v9]，音量由小到大。默认为中间值。 |
|          |      | 支持10级语速调节，发送协议头+[s0]-[s9]，语速由快到满，默认为中间值正常语速。 |
|          |      | 支持10级语调调节，发送协议头+[t0]-[t9]，语调由低到高，默认为中间值正常语调。 |
...
> 注意事项：
> 在windows设备中串口为COM+数字编号形式，如COM3；在Linux设备中USB串口为/dev/ttyUSB+数字编号，如/dev/ttyUSB0
